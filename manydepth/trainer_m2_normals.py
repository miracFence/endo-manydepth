...
class Trainer_Monodepth2:
    ...
    def compute_orth_loss(self, disp, N_hat, K_inv,ref_img):
        ...
        wandb.log({"D_inv_": wandb.Image(D_inv[0].transpose(2,0,1))},step=self.step)
        ...
        wandb.log({"D_inv_p": wandb.Image(D_inv_p[0])},step=self.step)
        ...
        for idx,p_idx in enumerate([-1,-2,-1,-2]):
            ...
            wandb.log({"D_inv_q": wandb.Image(D_inv_q[0])},step=self.step)
            #print(D_inv_q)
