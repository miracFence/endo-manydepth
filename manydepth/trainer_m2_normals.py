...
class Trainer_Monodepth2:
    ...
    def compute_orth_loss(self, disp, N_hat, K_inv):
        ...
        #print(N_hat.shape)
        Cpp = torch.einsum('bijk,bijk->bij', N_hat, X_tilde_p.view(batch_size,channels,height,width))
        for idx,p in enumerate(p_names[:4]):
            X_tilde_q = torch.matmul(K_inv[:, :3, :3], ps[p].to(device=K_inv.device))
            #print(X_tilde_q.shape)
            #print(N_hat.shape)
            Cpq = torch.einsum('bijk,bijk->bij', N_hat, X_tilde_q.view(batch_size,channels,height,width))
