...
class Trainer_Monodepth2:
    ...
    def compute_orth_loss3(self, disp, N_hat, K_inv):
        ...
        V = top_left_depth.view(batch_size,1,height,width) * pa_tl.view(batch_size,3,height,width) - bottom_right_depth.view(batch_size,1,height,width) * pb_br.view(batch_size,3,height,width)
        V += torch.abs(top_right_depth.view(batch_size,1,height,width) * pa_tr.view(batch_size,3,height,width) - bottom_left_depth.view(batch_size,1,height,width) * pb_bl.view(batch_size,3,height,width))
